...
def normalize_plain_button_text(student: StudentDB, message: str):
    ...
    lesson_already_done_today = (
        student.current_stage == 7
        and is_lesson_completed(student)
        and has_started_lesson_today(student)
    )
    aliases = {
    ...
    if lesson_already_done_today and text in {"continuar aula", "continuar"}:
        return message
...
def process_whatsapp_message(phone: str, message: str, db: Session):
    ...
    if student.current_stage == 7 and is_lesson_completed(student):
        if is_lesson_start_request(message) and not has_started_lesson_today(student):
            return start_guided_lesson(student, db, mode="manual")
...
"""
        ...
        level = get_openai_text(response)
    except Exception as error:
        print("Erro ao avaliar assessment. Usando fallback:", error)
        level = estimate_level_from_study_history(data.answer)
